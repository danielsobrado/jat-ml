...
REM --- Configuration ---
REM Set the path to the virtual environment created by uv
...
echo Starting RAG service locally using 'uv' virtual environment...
echo ==================================
...
REM 2. Check if the uv virtual environment exists
if not exist "%VENV_PATH%\Scripts\activate.bat" (
    echo ERROR: uv virtual environment not found at %VENV_PATH%
    echo Please run create-uv-rag.bat first to set up the environment.
    pause
    exit /b 1
)

REM 3. Set Environment Variables needed by the Python script
...
if "%PERSISTENT%"=="true" (
    ...
    if defined size (
        if !size! GTR 0 (
            ...
            REM Start ChromaDB server in a separate window using the uv environment
            echo Starting ChromaDB server with persistent storage at %CHROMA_DATA_DIR%...
            start "ChromaDB Server" cmd /K "call "%VENV_PATH%\Scripts\activate.bat" && chroma run --path %PROJECT_ROOT%\%CHROMA_DATA_DIR% --host %CHROMA_HOST% --port %CHROMA_PORT% || echo ChromaDB server failed to start! && pause"
...
REM Activate the virtual environment and run the main.py
...
REM Use the uv virtual environment to run the application
